...
const RestBar = ({ t, boxShadow = false }) => {
  ...
  return (
    <div className={clsx(style.Wrapper, boxShadow ? style.BoxShadow : null)}>
      <RestBarSetting open={visibleModal} closeModal={() => setVisibleModal(false)} />
      <div className={clsx(style.LeftWrapper)}>
        <div className={clsx(style.StressRisk)}>
          <div>
            <span className={clsx('text-uppercase font-heading-small')} style={{ opacity: 0.71 }}>
              {t('heat stress risk')}
            </span>
          </div>

          <div style={{ marginLeft: '25px' }}>
            <Button size="sm" rounded={true} color="green" bgColor="green" title={t('low')} />
          </div>
        </div>

        <div className={clsx(style.Schedule)}>
          <span className={clsx('font-binary')}>{t('Work a regular schedule')}</span>
        </div>
      </div>
      ...
      <div className={clsx(style.RightWrapper)}>
        <div className={clsx(style.TopWrapper)}>
          ...
          <div className={clsx(style.IconsWrapper)}>
            <div>
              <RoundIcon icon={clothIcon} customStyle={{ marginRight: '15px' }} />

              <RoundIcon icon={userSweatIcon} customStyle={{ marginRight: '15px' }} />
              {outdoor && <RoundIcon icon={sunExposeIcon} customStyle={{ marginRight: '15px' }} />}
            </div>
        ...
        <div className={clsx(style.BottomWrapper)}>
          <span className={clsx('font-helper-text')} style={{ opacity: '50%' }}>
