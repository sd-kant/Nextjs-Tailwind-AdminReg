...
const formSchema = (t) => {
  return Yup.object().shape({
    ...
    email: Yup.string()
      ...
      .test('required', t('email required'), function (value) {
        return this.parent.mode === 'email' ? !!value : true;
      }),
    phoneNumber: Yup.object()
      .test('required', t('phone number required'), function (obj) {
        return this.parent.mode === 'email' ? true : !!obj?.value;
      })
      .test('is-valid', t('phone number invalid'), function (obj) {
        return this.parent.mode === 'email'
          ? true
          : checkPhoneNumberValidation(obj.value, obj.countryCode);
      })
  });
};
    ...
    <Form className="form-group mt-57">
        ...
        <div className="grouped-form mt-25">
          <label className="font-binary d-block mt-8">
            {emailMode
              ...
              : t('forgot username phone description1')}
          </label>
        </div>
        <div className="grouped-form">
          <label className="font-binary d-block">
            {emailMode
              ...
              : t('forgot username phone description2')}
          </label>
        </div>
        {emailMode ? (
          ...
          <div className={clsx(style.PhoneNumberWrapper, 'd-flex flex-column mt-40')}>
            ...
            <CustomPhoneInput
              ...
              onChange={(value, countryCode) =>
                setFieldValue('phoneNumber', { value, countryCode })
              }
            />
      ...
      {status?.visibleModal && (
        <ConfirmModal
          ...
          header={
            emailMode
              ? t('forgot username confirm header')
              : t('forgot username phone confirm header')
          }
          subheader={
            emailMode
              ? t('forgot username confirm subheader')
              : t('forgot username phone confirm subheader')
          }
          onOk={(e) => {
            e.preventDefault();
