...
const FormMedical = (props) => {
  ...
  useEffect(() => {
    if (questionOrder !== null && medicalQuestions) {
      const currentQuestion =
        medicalQuestions &&
        medicalQuestions.find((question) => question.id?.toString() === questionOrder?.toString());
  ...
  const initialize = () => {
    ...
    const index =
      ...
      data.findIndex(
        (entity) =>
          entity.questionId === parseInt(order) &&
          entity.answerId &&
          entity.answerId.toString() !== 'null'
      );
  ...
  const goToPrev = () => {
    const prevQuestions =
      medicalQuestions && medicalQuestions.filter((question) => question.id < questionOrder);
  ...
  return (
    <div className="form-group mt-57">
      <div>
        <div className="d-flex align-center cursor-pointer" onClick={() => goToPrev()}>
        ...
        <div className="mt-28">
          <span className="font-binary d-block">{questionContent?.text}</span>
        </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslation()(FormMedical));
