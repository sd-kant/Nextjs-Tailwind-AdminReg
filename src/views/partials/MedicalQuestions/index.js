...
const MedicalQuestions = ({
  ...
  const onChangeOption = (questionId, value) => {
    const index = responses?.findIndex(
      (it) => it.questionId?.toString() === questionId?.toString()
    );
  ...
  if (!medicalQuestions) return null;

  return medicalQuestions.map((questionContent, index) => {
    const questionId = questionContent.id;
    const response = responses?.find(
      (entity) =>
        entity.questionId?.toString() === questionId?.toString() &&
        ![null, undefined, ''].includes(entity.answerId?.toString())
    );
...
export default connect(mapStateToProps, mapDispatchToProps)(MedicalQuestions);
