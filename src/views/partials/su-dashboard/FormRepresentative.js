...
import { INVALID_VALUES1, USER_TYPE_ORG_ADMIN } from '../../../constant';
    ...
    <Form className="form mt-57">
      <div style={{ padding: '0 10px' }}>
        ...
        <div className="grouped-form mt-40">
          <label className="font-header-medium">{t('company administrator')}</label>
        ...
        <div
          ...
          {orgAdmins?.map((user, index) => (
            <div
              ...
              <div className="d-flex flex-column ml-25">
                {index === 0 && (
                  <label className="font-input-label text-white">{t('administrator email')}</label>
                )}
          ...
          .finally(() => {
            if (totalSuccessForInvite > 0) {
              props.showSuccessNotification(
                props.t(
                  totalSuccessForInvite ? 'msg users created success' : 'msg user created success',
                  {
