...
const FormTeamCreate = (props) => {
  const { values, errors, touched, t, setRestBarClass, setFieldValue, isAdmin, organizationId } =
    props;
  ...
  useEffect(() => {
    ...
    getCompanyById(organizationId).then((response) => {
  ...
  const options = useMemo(
    () =>
      countryRegions
        ...
        ?.map((it) => ({ label: it.countryName, value: it.countryShortCode })),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <Form className="form mt-57">
      <div>
        ...
        <div className="grouped-form mt-40">
          <label className="font-header-medium">{t('create team')}</label>
  ...
  handleSubmit: async (values, { props }) => {
    ...
    try {
      ...
      props.showErrorNotification(
        e.response?.data?.validationErrors?.filter(
          (it) => it?.messageCode === 'error.team.duplicateName'
        )?.length > 0
          ? props.t('team already exist')
