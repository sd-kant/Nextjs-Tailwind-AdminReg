...
const EnhancedForm = withFormik({
  ...
  handleSubmit: async (values, {props}) => {
    ...
    try {
      ...
      props.showErrorNotification((e.response?.data?.validationErrors?.filter(it => it?.messageCode === "error.team.duplicateName")?.length > 0) ? "The team already exist" : e.response?.data?.message);
    } finally {
