...
const TableHeaderCellWrapper = ({ value, label, items }) => {
  ...
  const hideChevron = React.useMemo(
    () => ['connection', 'heatRisk', 'alerts'].includes(value),
    [value]
  );

  return (
    <TableHeaderCell value={value} label={formattedLabel} items={items} hideChevron={hideChevron} />
  );
};
