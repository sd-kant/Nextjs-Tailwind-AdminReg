...
const TableHeader = ({ columnsMap, visibleColumns, setVisibleColumns, forceWidthUpdate }) => {
  ...
  const items3 = React.useCallback(
    (header) => [
      {
        title: t('max to min'),
        action: () => {
  ...
  const items4 = React.useCallback(
    (header) => [
      {
  ...
  const items5 = React.useCallback(
    (header) => [
      {
        title: t('connected'),
        action: () => {
  ...
  const items1 = React.useCallback(
    (header) => [
      {
        title: t('most recent'),
        action: () => {
  ...
  const items2 = React.useCallback(
    (header) => [
      {
        title: t('high to low'),
        action: () => {
      ...
      <tr className={clsx(style.TableHeaderRow)}>
        ...
        {visibleColumns?.length < Object.keys(columnsMap)?.length + 1 && (
          <td className={clsx(style.TableHeaderCell)}>
            <DropdownMenu
              title={t('add columns')}
              icon={<img src={addIcon} alt="add column" />}
              items={menuItems}
            />
          </td>
        )}
