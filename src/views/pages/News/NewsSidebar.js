  ...
  return (
      <div className={clsx(style.SidebarBody)}>
        <div className={clsx(style.SearchBody)}>
          ...
          />
          <div className={clsx(style.SearchBtn, "capitalize")} onClick={() => setKeyword(keywordTmp ?? '')}>{t("search")}</div>
        </div>
            ...
            data?.posts?.map((item, key) => {
              ...
              return (
                  <div key={key} className={clsx(style.BlogLink)}>
                    <Link to={`/news/detail?id=${item?.id}`} className={id && Number(id) === item?.id ? clsx(style.Active) : ''}>
                      {item?.title ?? ''}
                    </Link>
                  </div>
              )
