  ...
  return (
    <div ref={chartRef} className={clsx(style.ChartBody)}>
      <div className={clsx(style.HighestCbtBody)}>
        <h1 className={clsx(style.TxtCenter)}>{t(`highest cbt by time of day and day of week`)}</h1>
        ...
        <div className={clsx(style.FlexSpace)}>
          ...
          <div className={clsx(style.CbtChartWRemain)}>
            {chartData?.dayList?.map((item, key) => {
              return (
                <div key={key} className={clsx(style.FlexSpace, style.DayItemBody)}>
                        ...
                        <div
                          ...
                          style={{
                            backgroundColor: `rgb(
                                    255, 
                                    ${
                                      chartData?.list?.length === 7 &&
                                      chartData?.list[key][index] !== null
                                        ? chartData?.list[key][index]?.maxCbtColor
                                        : 255
                                    }, 
                                    ${
                                      chartData?.list?.length === 7 &&
              ...
              <div className={clsx(style.CbtChartWRemain, style.FlexSpace)}>
                ...
                <div className={clsx(style.DayTimeGrid16)}>
                  {TIME_LIST?.map((time, index) => {
                    return (
                      <div key={index} className={clsx(style.TimeLabel, `mt-10`)}>
                        {time}
                      </div>
                    );
                  })}
