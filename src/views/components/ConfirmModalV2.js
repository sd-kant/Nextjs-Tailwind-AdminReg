      ...
      <div className={clsx(style.Modal, `z-index-2 ${show ? 'd-block' : 'd-none'}`)}>
        ...
        <div className={clsx(style.ModalBody, 'mt-60')}>
          <div className="d-flex justify-center">
            {visibleOk && (
              <div className={clsx(style.Tap, `cursor-pointer`)} onClick={onOk}>
                <img src={yesIcon} alt="male icon" />

                <span className="font-binary mt-8 text-uppercase">{okText ?? t('yes')}</span>
              </div>
            )}
            {visibleCancel && (
              <div className={clsx(style.Tap, `cursor-pointer ml-40`)} onClick={onCancel}>
